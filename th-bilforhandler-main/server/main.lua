...
RegisterNetEvent(
	...
	function(playerId, vehiclePrice, firstName, lastName, model, seller)
...
HT.RegisterServerCallback("th-bilforhandler:isPlateTaken", function(source, cb, plate)
	MySQL.scalar("SELECT plate FROM vrp_user_vehicles WHERE vehicle_plate = ?", { plate }, function(result)
		cb(result ~= nil)
	end)
end)
...
lib.callback.register(
	"dalle:addcar",
	function(source, name, model, price, category) -- Dette er crazy! lol this crazy frfr
		MySQL.Async.execute(
			"INSERT INTO th_vehicles (name, model, price, category) VALUES (@name, @model, @price, @category)",
			{
				["@name"] = name,
				["@model"] = model,
				["@price"] = price,
...
-- INSERT INTO `th_vehicles` (`name`, `model`, `prcie`, `category`) VALUES
